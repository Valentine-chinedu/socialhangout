...
const Post = ({ avatar, image, username, timestamp, message }) => {
	...
	return (
		<div className='flex h-auto justify-center items-start px-2 py-4 border-b hover:bg-gray-50 cursor-pointer'>
			...
			<div className='w-[88%] ml-4'>
				<div className='flex flex-col md:items-center md:flex-row '>
					<h3 className='mr-2 text-sm md:text-lg lg:text-sm font-bold text-gray-700'>
						{username}
					</h3>
					<p className='mr-2 opacity-70 lg:mt-1 text-xs md:text-sm font-medium text-gray-600'>
						{new Date(timestamp?.toDate()).toUTCString()}
					</p>
				</div>
				<div className='mr-2 mb-2 text-sm lg:mb-4 text-gray-700'>
					<p>{message}</p>
				</div>
				...
				{image && (
					<div className='mr-2 h-auto'>
						<img
							className='object-cover object-center rounded-3xl h-full w-'
							src={image}
							alt=''
						/>
					</div>
				...
				<div className='flex space-x-24 mt-4 pr-4'>
					<button className='focus:outline-none flex items-center'>
						<svg viewBox='0 0 24 24' aria-hidden='true' className='h-5 mr-2'>
							<g>
								<path
									fill='#686869'
